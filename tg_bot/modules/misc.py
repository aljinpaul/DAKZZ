...
SLAP_TEMPLATES = (
    "{user1} {user2} കാലിൽ തൂക്കി തറയിൽ അടിച്ചു കൊന്നു..😅",
    "{user1} കോടാലി കൊണ്ട് {user2} നെ രണ്ടായി വെട്ടിമുറിച്ചു..😂",
    ...
    "{user1} {user2} വിന്റെ തലക്ക് ഉലക്ക വെച്ചടിച്ചു..😜",
    "{user1} ഭിത്തിയിൽ തൂക്കിയിട്ടിരുന്ന 🕑 ക്ലോക്ക് എടുത്ത് {user2} വിന്റെ മൊട്ടത്തലയിൽ അടിച്ചു..😝😝",
...
def info(bot: Bot, update: Update, args: List[str]):
    ...
    if user.last_name:
        text += "\n🤦‍♂️Last Name: {}".format(html.escape(user.last_name))
...
def ping(bot: Bot, update: Update):
    ...
    bot.send_message(update.effective_chat.id, "⏳️Loading...!")
    end_time = time.time()
    ping_time = float(end_time - start_time)*1000
    update.effective_message.reply_text("⏱️ Ping speed was : {}ms".format(ping_time))
...
__help__ = """
 - /id: ഗ്രൂപ്പിന്റെ ഐഡി തരുന്നു. ഏതെങ്കിലും മെസ്സേജിന് മറുപടിയായി ഇട്ടാൽ ആ ഉപയോക്താവിന്റെ ഐഡി തരുന്നു.
 - /rmkeyboard: ചാറ്റിൽ നിന്നും ബോട്ട് കീബോർഡ് ഒഴിവാക്കുവാൻ വെണ്ടി.
 - /runs: ക്രമരഹിതമായവയും സെറ്റ് ചെയ്തതുമായ ഉദ്ധരണികൾ(വാചകങ്ങൾ) മറുപടിയായി തരുന്നു. 
 - /slap: ഉപയോക്താവിനെ അടിക്കുവാൻ വെണ്ടി. മറുപടി ആയി അയച്ചില്ലേൽ തിരിച്ചു കൊള്ളും. 
 - /time <place>: കൊടുത്തിരിക്കുന്ന സ്ഥലത്തിന്റെ പ്രാദേശിക സമയം നൽകുന്നു..
 - /info: ഉപയോക്താവിന്റെ എല്ലാ വിവരങ്ങൾക്കും വേണ്ടി.

 - /markdownhelp: markdown എങ്ങിനെ ടെലെഗ്രാമിൽ വർക്ക്‌ ചെയ്യുന്നു എന്നതിനുള്ള സഹായം (പ്രൈവറ്റ് ചാറ്റിൽ മാത്രം).
"""
